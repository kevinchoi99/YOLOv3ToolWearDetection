function [bboxes,scores,labels] = generateYOLOv3Detections(detections, confidenceThreshold, overlapThreshold, imageSize, classNames)
...
detections(:,6) = cellfun(@(a,b) reshapeClasses(a,b),detections(:,6),repmat({numel(classNames)}, size(detections(:,6))),'UniformOutput', false);
...
if ~isempty(detections)
    ...
    if ~isempty(detections)
        ...
        labels = categorical(classNames(labels));
